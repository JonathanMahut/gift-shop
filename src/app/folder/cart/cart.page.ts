...
export class CartPage implements OnInit {
    ...
    this.storage
      ...
      .then((data: itemCart[]) => {
        ...
        this.cartItems.forEach((element) => {
          ...
          this.total +=
            element.qty * element.item.price + element.item.availability.feed;
        });
      })
      .catch((err) => {
  ...
  async removeItem(article: itemCart, i: number) {
    const alert = await this.alertCtrl.create({
      ...
      message:
        'Etes-vous sûr de vouloir supprimer ' + article.item.title + ' ?',
      buttons: [
        {
          text: 'Annuler',
          role: 'cancel',
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.storage
              ...
              .then(async (data) => {
                // On supprime de IndexedDB
                const toast = await this.toastCtrl.create({
                  // On affiche un toast d'information
              ...
              .catch((err) =>
                console.log(
                  'Erreur durant la suppression de l\'article dans l\'IndexedDB ! : ',
                  err
                )
              );
